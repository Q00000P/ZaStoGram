...
- зависания в фазе `client_hello_sent` только логируются; штрафной cooldown
  временно выключен, чтобы клиент сам не переводил живые прокси в долгое
  состояние "недоступен" во время диагностики блокировок;
...
- `admission_grant`, `admission_queue`, `admission_release`,
  `admission_freeze_detected`, `admission_freeze_observed`.
